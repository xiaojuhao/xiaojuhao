...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__common_MyCabinSelect__ = __webpack_require__(703);
        ...
        querySearchAsync(queryString, cb) {
            ...
            this.timeout = setTimeout(() => {
                ...
                let result = this.allMaterialSupplier.map(item => {
                    let mt = __WEBPACK_IMPORTED_MODULE_0__common_bus__["a" /* api */].getSupplierFromStoreByCode(item.supplierCode);
                    console.log(mt);
                    let sk = item.materialName + "," + item.supplierName + "," + item.searchKey;
                    __WEBPACK_IMPORTED_MODULE_3_vue___default.a.set(item, "sk", sk);
                    if (mt && mt.shortName) {
                        __WEBPACK_IMPORTED_MODULE_3_vue___default.a.set(item, 'value', mt.shortName + "-" + item.materialName);
                    } else {
